...
from db import initDb, registerUser, loginUser, getUserProfile, updateUserProfile, giveBadge
from utils import *
import os
...
def register():
    ...
    if status == 201:
        ...
        giveBadge(session['userid'], "siege", "Siege", "Signed up during the Siege weeks!")
...
def editprofile():
    if 'userid' in session:
        data = request.get_json()
        username = data.get('username')
        result, status = updateUserProfile(session['userid'], username=username)
        return jsonify(result), status
